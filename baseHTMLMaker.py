...
class Stylesheet(HTMLTag):
    def __init__(self, cont: str, issrc: bool = False, **kwargs):
        super().__init__('link'*issrc+'style'*(not issrc), cont*(not issrc), href=cont*issrc, rel='stylesheet'*issrc, type='text/css', **kwargs)
...
def groupGCharts(charts: list[list]):
    return {'head': [Script('https://www.gstatic.com/charts/loader.js', True)]+[i[0] for i in charts], 'body': [i[1] for i in charts]}

basis = {'head': [HTMLTag('meta', charset='utf-8'), HTMLTag('title', 'Polfore')], 'body': []}
